...
def test(model, test_data, test_loader, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            if i % 20 == 0:
                ...
                visual(gt, pd, os.path.join('pic/', str(i) + '.png'))
    ...
    print('mae:{:.7f}, mse:{:.7f}, rmse:{:.7f}, smape:{:.7f}'.format(mae, mse, rmse, smape))
    f = open("result_long_term_forecast.txt", 'a')
    f.write('{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_gl{}_df{}_eb{}'.format(args.model_id, 336, args.label_len, args.pred_len,
                                                                        args.d_model, args.n_heads, args.e_layers, args.gpt_layers,
                                                                        args.d_ff, args.embed + "  \n"))
    f.write('mae:{:.7f}, mse:{:.7f}, rmse:{:.7f}, smape:{:.7f}'.format(mae, mse, rmse, smape))
    f.write('\n')
    f.write('\n')
    f.close()
